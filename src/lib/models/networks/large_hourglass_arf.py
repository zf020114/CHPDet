...
class exkp(nn.Module):
    def __init__(
        ...
        #TODO insert
        self.feat_channels=256#channels[self.first_level]
        self.or_conv1 = ORConv2d(self.feat_channels, int(
                self.feat_channels ), kernel_size=3, padding=1, arf_config=(1, 8))
        self.or_conv2 = ORConv2d(self.feat_channels, int(
                self.feat_channels ), kernel_size=3, padding=1, arf_config=(1, 8))
    ...
    def forward(self, image):
        ...
        for ind in range(self.nstack):
            ...
            if ind ==0:
                cnv=self.or_conv1(cnv)
            else:
                cnv=self.or_conv2(cnv)
            cnv= self.or_pool(cnv)
            out = {}
